...
class TokenHelper:
    ...
    def create_token(
        ...
        expire = datetime.now(UTC) + timedelta(
            minutes=cls._resolve_expiry_minutes(token_type, expires_delta)
        )

        to_encode.update({"exp": expire, "type": token_type.value})
        return jwt.encode(
            to_encode,
            cls._resolve_secret_key(token_type),
    ...
    @staticmethod
    def _resolve_expiry_minutes(
        token_type: TokenType, expires_delta: int | None
    ) -> int:
